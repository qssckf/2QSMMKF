...
public class PreOrderExecQuery extends BillQuery{
	...
	public List<Map<String, Object>> queryByPks(String[] pks) throws BusinessException {
		...
		String sql="select substr(a.dbilldate,0,10) billdate,ks.name ksname,mat.name matname,mat.materialspec,kz.name kzname,sum(b.nnum) ynum,sum(nvl(c.nnum,0)) fnum,sum(b.nastnum) ynastnum,sum(nvl(c.nastnum,0)) fnastnum,sum(b.nnum)-sum(nvl(c.nnum,0)) wfnum,sum(b.nastnum)-sum(nvl(c.nastnum,0)) wfnastnum from so_preorder a";
		...
		sql=sql+" where a.cpreorderid in "+inSql;
		sql=sql+" group by substr(a.dbilldate,0,10),ks.name,mat.name,mat.materialspec,kz.name";
		...
		while(row.next()){
			...
			item.put("prenum", row.getString(5));
			item.put("fhnum", row.getString(6));
			item.put("prenastnum", row.getString(7));
			item.put("fhnastnum", row.getString(8));
			item.put("wfnum", row.getString(9));
			item.put("wfnastnum", row.getString(10));
	...
	private String getBaseSql() {
		...
		if (!StringUtil.isEmptyWithTrim(getCondition()))
	    {
	      ...
	      String sql = "select so_preorder.cpreorderid from so_preorder left join so_preorder_b on so_preorder.cpreorderid=so_preorder_b.cpreorderid where so_preorder.dr=0 and so_preorder_b.dr=0 and so_preorder.pk_group='#pk_group#' and so_preorder.billmaker='#billmaker#' and so_preorder.dr=0 and so_preorder_b.dr=0 and ("+this.getCondition()+") order by so_preorder.dbilldate desc";
	
	      return sql;
	    }
	    return "select so_preorder.cpreorderid from so_preorder left join so_preorder_b on so_preorder.cpreorderid=so_preorder_b.cpreorderid where so_preorder.dr=0 and so_preorder_b.dr=0 and so_preorder.pk_group='#pk_group#' and so_preorder.billmaker='#billmaker#' and so_preorder.dr=0 and so_preorder_b.dr=0 order by so_preorder.dbilldate desc";
